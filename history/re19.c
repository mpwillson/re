...
expr(void)
{
    ...
    if (expr_start == 1) {
        ...
        if (debug) printf("expr final: last_expr_start: %2d, dot_state: %2d\n",
                          last_expr_start, dot_state);
...
factor(void)
{
    ...
    if (c == RE_LP) {
        ...
        expr();
        if (debug) printf("( postamble: expr_final_state: %2d, "
                          "nested_expr_start: %2d\n",
                          expr_final_state, nested_expr_start);
        ...
        if (dot_state > 0 && get_state(end_state-1)->event == RE_CL) {
            ...
            // disregard dot_state if not closure related
            //dot_state = -1;
    ...
    if (c == RE_CL) {
        if (st->event > '\0') {
            ...
            if (debug)
                printf("dot closure: dot_state: %2d,end_state: %2d\n",
                       dot_state, end_state);
        ...
        insert_state(end_state,RE_CL,end_state+1,end_state+1);
        end_state++;
    }
    else {
        unlexch();
    ...
    while (!endstr) {
        ...
        while (st->event <= '\0'  && st->event != RE_DOT) {
            ...
            if (st->event <= '\0') {
                ...
                if (st->event == RE_DOT &&
                    ...
                    if (debug) printf("dot/closure target: i: %2d, "
                                      "back_track: %2d\n",
                                      i, back_track);
                    i++;
                }
        ...
        if (s[i] == st->event || st->event == RE_DOT) {
            ...
            if (!follow_next2 || (st->next2 > state &&
                ...
                // turn off anchors that belong to this expr
