...
factor(void)
{
    ...
    if (c != RE_CL) {
        ...
        if (get_state(state-1)->event == RE_DOT)
            insert_state(state,'\0', t2, state+1);
        else
...
matcher(struct s_entry* machine, char* search_str, int start)
{
    ...
    while (state) {
        ...
        if (state == SCAN) {
            ...
            if (st->next2 == RE_BOL && j != 0) return NO_MATCH;
            ...
            if (st->next1 == state+1)
                get_state(state+1)->next2 = RE_BOL; // applies this expr
            else
        ...
        if (debug) dq_print();
        //if (dq_empty() || j == N) return NO_MATCH;
        if (dq_empty() || j == N) return NO_MATCH;
        state = dq_pop_head();
