...
expr(void)
{
    ...
    if (lexch() == RE_OR) {
        ...
        if (get_state(end_state-1)->event < 0)
            // fail state for previous expression
...
factor(void)
{
    ...
    if (c == RE_LP) {
        ...
        insert_state(end_state, '\0', end_state+1, -1);
        end_state++;
        expr();
        if (debug)
        ...
        if (c == RE_RP) {
            ...
            insert_state(end_state,-1,end_state+2,end_state+2);
            insert_state(end_state++,-1, -1, last_expr_start);
            // fail of previous expression goes to this fail state
            get_state(last_expr_start)->next2 = end_state++;
        }
        else {
            unlexch();
        }
        ...
        if (st->event > 0) {
            ...
            if (debug)
                printf("last_expr_state+1(%2d)->next1 = end_state (%2d)\n",
                       last_expr_start+1, end_state);
            get_state(end_state-1)->next1 = end_state;
                //}
