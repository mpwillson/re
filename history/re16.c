 ...
 *
 * Version 16 - (based on 15) Use expr_final_state global to save and
 * access start of last expr in an OR set (or only expr if not an OR).
 *
 */
...
expr(void)
{
    ...
    if (c == RE_OR) {
            ...
            complete_state = end_state-1;  //TBD
            expr();
            if (debug)
            ...
            get_state(complete_state)->next1 = end_state-1; //-1; //TBD ? -1
    }
    else {
...
factor(void)
{
    ...
    if (c == RE_CL) {
        if (st->event > '\0') {
            ...
            // use last char to return to start, not complete state
            // as complete state be munged by a following OR
            get_state(end_state-1)->next1 = nested_expr_start+1;
            // failure on first closure character goes to end state to
            // carry on (for simple sequence of characters)
            //get_state(get_state(end_state-1)->next2+1)->next2 =
            // end_state;
            get_state(expr_final_state)->next2 = end_state;
        }
...
match(char* s)
{
    ...
    while (!endstr) {
        ...
        if (s[i] == st->event) {
            ...
            if (follow_next2 && get_state(new_state)->next2 > state) {
                // OR
                if (back_track >= 0 ) {
