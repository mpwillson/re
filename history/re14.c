 ...
 *
 * Version 14 - (based on 13) attempt removal of extraneous states
 *  Won't work; or exprs need a final complete state so that the
 *  successful or match can go to end.
 *
 *
 */
...
expr(void)
{
    ...
    if (expr_start == 1) {
        ...
        // fake brackets
        // last expression fails to fake expr start 0-
...
factor(void)
{
    ...
    if (c == RE_CL) {
        if (st->event > '\0') {
            ...
            st->next1 = nested_expr_start; //end_state-1;
            if ((end_state - nested_expr_start) == 1)
                st->next2 = end_state;
            else
                get_state(get_state(end_state-1)->next2+1)->next2 = end_state;
        }
        else {
            if (debug) printf("closure of bracketed expr\n");
            // successful end returns to start of first contained expr
            st->next1 = nested_expr_start+1;
