...
expr(void)
{
    ...
    if (expr_start == 0) {
        if (!insert_state(expr_start++, '\0', 1, -1)) error(RE_ERR_MEM);
        end_state++;
    }
    if (!insert_state(expr_start, '\0', ++end_state, current_fail_state))
        error(RE_ERR_MEM);
    ...
    if (expr_start == 1) {
        ...
        st->next2 = (dot_state < 0)?expr_start-1:dot_state;
        if (!insert_state(end_state,'\0',0,0)) error(RE_ERR_MEM);
    }
        ...
        if (!insert_state(end_state,'\0',end_state+1,expr_start))
            error(RE_ERR_MEM);
        end_state++;
    }
...
factor(void)
{
    ...
    if (c == RE_LP) {
        ...
        DEBUG("factor: ( preamble", current_fail_state, nested_expr_start, 0);
        if (!insert_state(nested_expr_start,'\0',++end_state,
        ...
        if (dot_state > 0 && get_state(end_state-1)->event == RE_CL) {
            // closure of dot preceeded this char
            if (!insert_state(end_state, c, end_state+1, dot_state))
                error(RE_ERR_MEM);
             dot_state = -1;
        }
        else {
            if (!insert_state(end_state, c, end_state+1, current_fail_state))
                error(RE_ERR_MEM);
        }
        end_state++;
    }
    ...
    if (c == RE_CL) {
        if (st->event > '\0') {
            ...
            DEBUG("dot closure", dot_state, end_state, 0);
        ...
        if (!insert_state(end_state,RE_CL,end_state+1,end_state+1))
            error(RE_ERR_MEM);
        end_state++;
    }
    else {
...
struct re_matched*
re_match(struct s_entry* machine, char* s)
{
    struct s_entry* st;
    ...
    while (!endstr) {
        ...
        while (st->event <= '\0'  && st->event != RE_DOT) {
            ...
            if (st->event <= '\0') {
                ...
                if (st->event == RE_DOT &&
                    get_state(new_state+1)->event == RE_CL) {
        ...
        if (s[i] == st->event || st->event == RE_DOT) {
            ...
            follow_next2 = (new_state < state) &&
                (get_state(new_state)->event != RE_DOT);
            st = get_state(new_state);
            DEBUG("fail", new_state, 0, 0);
            if (!follow_next2 || (st->next2 > state &&
                                  get_state(st->next2)->event == RE_CL)) {
                DEBUG("closure", i, 0, 0);
            }
            else {
                // only backtrack for alternates (not closures)
                // turn off bol_anchor that belong to this expr
                if (bol_anchor == get_state(state)->next2) bol_anchor = 0;
                if (back_track >= 0) {
