  ...
  start:
        ...
        case RE_LP:
            ...
            if (c == RE_RP) {
                ...
                if (c == RE_CL) {
                    ...
                    get_state(end_state-1)->next1 = nested_expr_start;
    ...
    if (debug)
        printf("expr end: expr_start: %2d, end_state: %2d\n",
               expr_start, end_state);
    if (expr_start == 1) {
        // outmost expression; fake brackets
...
factor(void)
{
    ...
    if (c > '\0')  {
        ...
        // otherwise, copy fail state from previous
        fail_state = (st->event == '\0')?end_state-1:st->next2;
